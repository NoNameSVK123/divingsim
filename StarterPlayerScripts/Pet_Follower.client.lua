...
function GridPositionGen(numOfPets, petIndex, playerHrp) :Vector3
	...
	temp,horizontalOffset,backwardsOffset=nil
	return offset
...
local function positionPets(character, folder, dt) 
	...
	for i, pet in pairs(folder:GetChildren()) do
		if pet:IsA("Model") then
			if pet:FindFirstChild("lerp") then
				local lerp = pet:FindFirstChild("lerp").Value
				if lerp==false then
					lerp=true
					...
					if pet:FindFirstChild("Attack").Value ~= nil then
						...
						if pet:WaitForChild("aligned").Value==false then
							...
							local s =soundservice:WaitForChild("PetSpawn"):Clone()
							s.Parent=pet.PrimaryPart
							s:Play()
							...
							task.delay(1,function()
								if pet:FindFirstChild("spawnParticle") then
				...
				local lerp = Instance.new("BoolValue")
				lerp.Name = "lerp"
				lerp.Value=false
				lerp.Parent=pet
			end
...
local function positionPetsUNDERWATER(character, folder, dt)
	petsound = math.random(1,3)
	for i, pet in pairs(folder:GetChildren()) do
		if pet:IsA("Model") then
			local radius = 4+#folder:GetChildren()
			...
			if pet:FindFirstChild("Attack").Value ~= nil then
				...
				if character.Humanoid.MoveDirection.Magnitude > 0 then
					if pet.PrimaryPart~=nil then
						...
						if pet:FindFirstChild("Walks") then
							pet:SetPrimaryPartCFrame(pet.PrimaryPart.CFrame:Lerp(character.PrimaryPart.CFrame * CFrame.new(x, 1 +sin, z) * CFrame.fromEulerAnglesXYZ(0,0,cos),smoothness*dt))
						else
							pet:SetPrimaryPartCFrame(pet.PrimaryPart.CFrame:Lerp(character.PrimaryPart.CFrame * CFrame.new(x, 3+math.sin(time()*7)/2, z),smoothness*dt))
						end 
					end	
				else
					if pet.PrimaryPart~=nil then
						if pet:FindFirstChild("Walks") then 
							pet:SetPrimaryPartCFrame(pet.PrimaryPart.CFrame:Lerp(character.PrimaryPart.CFrame * CFrame.new(x, 1, z) ,smoothness*dt))
						else
							pet:SetPrimaryPartCFrame(pet.PrimaryPart.CFrame:Lerp(character.PrimaryPart.CFrame * CFrame.new(x, 3+math.sin(time()*7)/2, z) ,smoothness*dt))
						end
					end
		...
		if Player ~= nil then
			...
			if character ~= nil then
				if character:FindFirstChild("HumanoidRootPart") then
					if character.HumanoidRootPart.Position.Y<200 then
						positionPetsUNDERWATER(character, PlrFolder, dt)
					else
